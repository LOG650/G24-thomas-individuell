...
The ABC analysis assigns 182 articles (25.7 %) to category A, accounting for approximately 80 % of total value. K-means identifies K = 3 clusters with silhouette scores of 0.383 (training) and 0.368 (test), confirming acceptable cluster structure. The rule engine recommends transfer of 145 articles (20.5 %), while 257 are retained locally, 284 require manual review, and 23 lack sufficient data. The estimated annual savings potential is NOK 451 515 in the base case (realisation rate g = 75 %), ranging from NOK 301 010 (pessimistic) to NOK 602 020 (optimistic) across sensitivity scenarios.

The study concludes that multi-criteria classification combined with cluster analysis of SAP transaction data provides an operationally applicable basis for identifying HVFS transfer candidates, but realisation requires active adjustment of SAP MM parameters and structured expert review of the REVIEW group.
