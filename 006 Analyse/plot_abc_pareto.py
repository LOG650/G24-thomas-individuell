"""
Genererer Fig04_ABC_Pareto.png
...
ax1.axvline(x=idx_ab, color=C_A, linestyle="--", linewidth=1.2, alpha=0.6,
            zorder=3)
ax1.axvline(x=idx_bc, color=C_B, linestyle="--", linewidth=1.2, alpha=0.6,
            zorder=3)
...
ax1.grid(axis="y", alpha=0.20, linewidth=0.6, zorder=0)
...
legend_elements = [
    Patch(facecolor=C_A, alpha=0.75,
          label=f"A: {n_a} artikler ({pct_a:.0f} %) – 80 %"),
    Patch(facecolor=C_B, alpha=0.75,
          label=f"B: {n_b} artikler ({pct_b:.0f} %) – 15 %"),
    Patch(facecolor=C_C, alpha=0.75,
...
# ── Eksporter ────────────────────────────────────────────────────
plt.tight_layout()
out = r"C:\G24\G24-thomas-individuell\006 analyse\plots\Fig04_ABC_Pareto.png"
