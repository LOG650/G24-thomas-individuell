...
| **Dato:** | 2026-02-24 |
| **Versjon:** | 1.0 – synkronisert med SAP-uttrekk v1.0 |
...
| FK-08 | Validering: samsvar mellom SAP-XYZ (MDMA.ZZXYZ) og beregnet XYZ fra MSEG | Må |
...
- Analyseperiode begrenset til 2024–2025 (MSEG.BUDAT, EKKO.BEDAT).
...
- **Datainnhenting og forbehandling:** Import av 15 Excel-filer fra SE16H-uttrekk (jf. Vedlegg A, seksjon A.5). Kobling på MATNR som primærnøkkel. Datavask inkluderer PEINH-korreksjon, håndtering av null-priser og nullforbruk. MSEG-filer (2024 og 2025) slås sammen med pd.concat() før aggregering.
- **ABC-analysemodul:** Klassifisering basert på kumulativ innkjøpsverdi fra EKPO.NETWR, med Pareto-fordeling (A: 80 %, B: 15 %, C: 5 %).
- **XYZ-klassifiseringsmodul:** Beregning av variasjonskoeffisient (CV) på månedlig forbruk aggregert fra MSEG (BWART=201+647) per MATNR per måned (BUDAT), med terskler X < 0,5, Y: 0,5–1,0, Z > 1,0. Validering mot MDMA.ZZXYZ (Helse Bergens operative XYZ-indikator).
- **EOQ-avviksmodul:** Beregning av Wilson EOQ med årlig etterspørsel (D) fra MSEG (faktisk forbruk ut fra LGORT 3000), bestillingskostnad (S) = 500–1 000 kr, og holdekostnad (H) = enhetspris (MBEW.STPRS/PEINH) × holdekostnadssats (20 %, sensitivitet 15–25 %). Identifisering av artikler med signifikant avvik mellom faktisk og optimal bestillingsfrekvens.
...
| 3.2x | Datautrekk fra SAP iht. spesifikasjon (15 filer) | Fase 3 | Vedlegg A |
...
En sentral kvalitetsindikator er samsvar mellom SAP-lagret XYZ-klassifisering (MDMA.ZZXYZ – Helse Bergens operative indikator) og prosjektets beregnede XYZ fra MSEG-forbruksdata. MARC.MTVFP (SAP standard) beholdes som sekundær referanse for sammenligning. Dette måles gjennom:

- **XYZ-dekningsgrad:** andel artikler med MDMA.ZZXYZ tilgjengelig (ikke-blank).
- **Samsvarsprosent:** andel artikler der beregnet XYZ stemmer overens med MDMA.ZZXYZ.
...
Komplett dataspesifikasjon for SE16H-uttrekk fra Helse Bergen forsyningslager. Fabrikk (WERKS): 3300, Lagersted (LGORT): 3000. Versjon 1.0 – synkronisert med SAP-uttrekk v1.0.

### A.1 Tabelloversikt og dataflyt

**Viktig distinksjon:**
- 🔴 **MSEG** = forbruk **ut** fra lager til avdeling → XYZ/CV og EOQ etterspørsel D
- 🔵 **EKKO/EKPO/EKBE** = innkjøp **inn** til lager fra leverandør → ABC-verdi og EOQ-avvik

Følgende 14 SAP-tabeller dekker alle beslutningsvariabler i prosjektet.

| **#** | **Tabell** | **Innhold** | **Brukes til** | **Status** |
|-------|-----------|-------------|----------------|------------|
| 1 | MARD | Lagerbeholdning per lagersted | Definerer artikkelunivers (LGORT 3000) | ✓ Kritisk |
| 2 | MDMA | ABC/XYZ-indikatorer per MRP-område (ZZABC, ZZXYZ) | Operativ XYZ-validering – Helse Bergens eget system | ✓ Kritisk |
| 3 | MARA | Materialtype, varegruppe, basisenhet | Filtrere forbruksmateriell, ekskludere legemidler | ✓ Kritisk |
| 4 | MAKT | Materialebeskrivelse (klartekst) | Lesbar output | ✓ Kritisk |
| 5 | MARC | SAP standard XYZ (MTVFP), MRP-parametere | Sekundær XYZ-referanse, sikkerhetslager, EOQ/ROP | ✓ Kritisk |
| 6 | MBEW | Enhetspris (standard/glidende), prisenhet | ABC-verdi og EOQ holdekostnad (H = pris × sats) | ✓ Kritisk |
| 7 | MSEG 2024 | Varebevegelser BWART=201+647 | 🔴 Forbruk ut – XYZ/CV og EOQ etterspørsel D | ✓ Kritisk |
| 8 | MSEG 2025 | Varebevegelser BWART=201+647 | 🔴 Forbruk ut – XYZ/CV og EOQ etterspørsel D | ✓ Kritisk |
| 9 | EKKO | Innkjøpsordrehode – dato, type | Filtrere EKPO på 2024–2025, BSART=ZNB, EKGRP=3000+300 | ✓ Kritisk |
| 10 | EKPO | Innkjøpsordreposisjon – kvantum og pris | 🔵 ABC-verdi, ordrefrekvens, EOQ-avvik | ✓ Kritisk |
| 11 | EKBE | Faktiske varemottak per ordre (BWART=101) | 🔵 Netto mottatt – EOQ-avvik (bestilt vs mottatt) | ✓ Kritisk |
| 12 | EINA | Leverandør-artikkel-infopost | Kobling MATNR → INFNR → EINE | Bør ha |
| 13 | EINE | Leveringstid per infopost (WEBAZ) | EOQ / ROP | Bør ha |
| 14 | T023T | Varegruppenavn (klartekst) | Lesbar gruppering i output | Bør ha |

### A.2 Seleksjonskriterier i SE16H

| **Parameter** | **Verdi** | **SAP-felt** | **Merknad** |
|--------------|----------|-------------|-------------|
| Anlegg | 3300 | WERKS | MARC, MBEW, MARD, MSEG, EKPO, EINA |
| Lagersted | 3000 | LGORT | MARD, MSEG, EKPO |
| MRP-område | 3000 | BERID | MDMA – tilsvarer LGORT 3000 |
| Vurderingsområde | 3300 | BWKEY | MBEW |
| Bevegelsestype forbruk | 201, 647 | BWART | MSEG – vareuttak og levering til transitt |
| Ordrekategori | F | BSTYP | EKKO |
| Bestillingstype | ZNB | BSART | EKKO – Helse Bergens bestillingstype |
| Innkjøpsgruppe | 3000 og 300 | EKGRP | EKKO – identifiserer forsyningslageret |
| Bevegelseskategori | E | BEWTP | EKBE – faktiske varemottak |
| Bevegelsestype mottak | 101 | BWART | EKBE – ekskluderer 102 reverseringer |
| Sletteflagg | blank | LOEKZ | EKPO |
| Språk | NO | SPRAS | MAKT, T023T |
| Materialtype | Avklar med HB | MTART | MARA |

### A.3 Koblingsnøkler

| **Fra tabell** | **Via felt** | **Til tabell** | **Formål** |
|---------------|-------------|---------------|-----------|
| MARD | MATNR | MDMA | Bekreft aktive artikler i MRP-område 3000 |
| MDMA | MATNR | Alle tabeller | Primær MATNR-filterliste fra nr. 3 og utover |
| MSEG | MATNR (aggregert/mnd) | Masterfile | Månedlig forbruk, CV, EOQ etterspørsel D |
...
|-------------|----------------|------------------------|
| Artikler uten pris | MBEW.STPRS = 0 eller null | Ekskluder fra ABC |
...
| Prisenhet ≠ 1 | MBEW.PEINH = 10 eller 100 | Korriger: pris = STPRS ÷ PEINH |
| EKBE SHKZG | SHKZG = H > 20 % | Summer netto: S minus H |
...
> **Viktig:** PEINH i MBEW er ofte 10 eller 100 – ikke 1. Pris per stykk = STPRS ÷ PEINH. Hyppig årsak til feil ABC-beregning.

### A.5 Uttrekksrekkefølge og bekreftet radvolum

| **#** | **Tabell** | **Filnavn** | **Filter** | **Rader** |
|-------|-----------|-------------|-----------|-----------|
| 1 | MARD | MARD_3300_3000.xlsx | WERKS=3300, LGORT=3000 | – |
| 2 | MDMA | MDMA_3300_3000.xlsx | MATNR fra MARD, WERKS=3300, BERID=3000 | – |
| 3 | MARA | MARA_3300.xlsx | MATNR fra MDMA, MTART=[avklar] | – |
| 4 | MAKT | MAKT_NO.xlsx | MATNR fra MDMA, SPRAS=NO | – |
| 5 | MARC | MARC_3300.xlsx | MATNR fra MDMA, WERKS=3300 | – |
| 6 | MBEW | MBEW_3300.xlsx | MATNR fra MDMA, BWKEY=3300 | – |
| 7 | MSEG 2024 | MSEG_3300_3000_2024.xlsx | MATNR fra MDMA, WERKS=3300, LGORT=3000, BWART=201+647, BUDAT=2024 | ~135 000 |
| 8 | MSEG 2025 | MSEG_3300_3000_2025.xlsx | MATNR fra MDMA, WERKS=3300, LGORT=3000, BWART=201+647, BUDAT=2025 | ~135 000 |
| 9 | EKKO | EKKO_2024_2025.xlsx | BSTYP=F, BSART=ZNB, EKGRP=3000+300, BEDAT=2024–2025 | 13 154 |
| 10 | EKPO | EKPO_3300.xlsx | EBELN fra EKKO, MATNR fra MDMA, WERKS=3300, LGORT=3000 | 8 240 |
| 11 | EKBE | EKBE_2024_2025.xlsx | EBELN+EBELP fra EKPO, BEWTP=E, BWART=101, BUDAT=2024–2025 | 9 004 |
| 12 | EINA | EINA_3300.xlsx | MATNR fra MDMA, WERKS=3300 | – |
| 13 | EINE | EINE_3300.xlsx | INFNR fra EINA | – |
| 14 | T023T | T023T_NO.xlsx | MATKL fra MARA, SPRAS=NO | – |
